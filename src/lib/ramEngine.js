export class RAMEngine {
  ...
  reset() {
    this.memory = Array(64).fill(0); // Arbitrary initial size, can expand
    this.acc = 0;
    this.ip = 0;
    this.inputTape = [];
    this.outputTape = [];
    this.program = []; // Array of { ln, label, instruction, argument, comment }
    this.labels = {}; // Maps label string to LN (line number/index)
    this.halted = false;
    this.error = null;
  }
      ...
      switch (inst) {
        case 'READ': {
          ...
          if (this.inputTape.length === 0) {
             // In some simulators it halts or reads 0. Let's read 0.
